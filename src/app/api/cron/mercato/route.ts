...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const { league, feeds } of LEAGUE_FEEDS) {
      ...
      // Se l'insieme dei titoli è già stato elaborato, non richiamiamo Gemini.
      // Il job può quindi controllare i feed ogni 30 minuti senza generare costi
      // quando non è arrivata nessuna novità.
