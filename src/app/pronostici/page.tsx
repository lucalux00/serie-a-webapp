...
import { Target, ExternalLink, Calculator, AlertTriangle, Loader2, ChevronDown, ChevronUp, Info, CalendarClock, BrainCircuit, Lock } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { getTeamLogoUrl } from '@/utils/teamLogos';
import { useAuth } from '@/context/AuthContext';

type Prediction = { id: string; match: string; competition: string; pick: string; odds: number; commence_time?: string };
type Multiple = { id: string; title: string; description: string; matches: Prediction[]; totalOdds: number };
type CompetitionBoard = { competition: string; singles: Prediction[]; multiples: Multiple[]; totalMatches: number };
...
export default function PronosticiPage() {
  ...
  // MLOps State
  const [competitionBoards, setCompetitionBoards] = useState<CompetitionBoard[]>([]);
    ...
    if (mainTab === 'mlops' && !mlopsLoaded) {
      ...
      async function loadPredictions() {
        try {
          ...
          if (res.ok) {
            const data = await res.json();
            setCompetitionBoards(data.competitions || []);
          }
        } catch (error) {
  ...
  const availableCompetitions = useMemo(() => ['Tutti', ...competitionBoards.map((board) => board.competition)], [competitionBoards]);
  const filteredBoards = useMemo(() => activeTab === 'Tutti' ? competitionBoards : competitionBoards.filter((board) => board.competition === activeTab), [activeTab, competitionBoards]);
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            ...
            {loadingMlops ? (
              ...
              <>
                ...
                <div className="mb-8 rounded-2xl border border-[#0EA5E9]/25 bg-[#0EA5E9]/10 p-4 text-sm text-[#cbd5e1]">
                  Ogni campionato ha le sue <strong className="text-white">singole</strong> e le sue <strong className="text-white">multiple</strong>: una multipla non mescola mai partite di leghe diverse.
                </div>

                <div className="space-y-8">
                  {filteredBoards.length ? filteredBoards.map((board) => (
                    <section key={board.competition} className="rounded-3xl border border-[#334155] bg-[#162033]/60 p-4 sm:p-5">
                      <div className="mb-5 flex items-center justify-between gap-3">
                        <div><h3 className="flex items-center text-xl font-black text-white"><Target className="mr-2 text-[#10B981]" size={20} />{board.competition}</h3><p className="mt-1 text-xs text-[#94A3B8]">Le migliori selezioni disponibili per questo campionato.</p></div>
                        <span className="rounded-lg bg-[#0F172A] px-2 py-1 text-xs font-bold text-[#94A3B8]">{board.totalMatches} match</span>
                      </div>

                      <h4 className="mb-3 text-sm font-black uppercase tracking-wider text-[#10B981]">Pronostici singoli</h4>
                      <div className="grid gap-3 lg:grid-cols-2">
                        {board.singles.map((pred) => <motion.article key={pred.id} layout className="overflow-hidden rounded-xl border border-[#334155] bg-[#1E293B] shadow-sm"><button type="button" className="flex w-full items-center justify-between gap-3 p-4 text-left transition-colors hover:bg-[#334155]/40" onClick={() => toggleMlopsAnalysis(pred)}><div className="min-w-0"><p className="font-black text-sm text-[#F8FAFC]">{pred.match}</p><div className="mt-1 flex flex-wrap gap-2 text-[10px] font-bold uppercase tracking-wider text-[#94A3B8]"><span>Esito: <span className="text-[#10B981]">{pred.pick}</span></span>{pred.commence_time && <span>{formatDate(pred.commence_time)}</span>}</div></div><div className="flex shrink-0 items-center gap-2"><span className="rounded-lg border border-[#334155] bg-[#0F172A] px-2 py-1.5 font-black text-white">{pred.odds.toFixed(2)}</span>{expandedMlopsAnalysis === pred.id ? <ChevronUp size={18} className="text-[#94A3B8]" /> : <ChevronDown size={18} className="text-[#94A3B8]" />}</div></button><AnimatePresence>{expandedMlopsAnalysis === pred.id && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-[#334155]/50 bg-[#0F172A]/50 p-4">{mlopsExplanations[pred.id]?.loading ? <Loader2 className="mx-auto animate-spin text-[#10B981]" size={24} /> : mlopsExplanations[pred.id]?.locked ? <div className="text-center"><Lock size={20} className="mx-auto mb-2 text-[#10B981]" /><p className="text-sm font-black text-white">Analisi Premium</p><a href="/profilo" className="mt-3 inline-flex rounded-lg bg-[#10B981] px-3 py-2 text-xs font-black text-[#0F172A]">{user ? 'Passa a Premium' : 'Accedi'}</a></div> : <div className="prose prose-invert max-w-none text-sm text-[#cbd5e1]" dangerouslySetInnerHTML={{ __html: mlopsExplanations[pred.id]?.text }} />}</motion.div>}</AnimatePresence></motion.article>)}
                      </div>

                      <h4 className="mb-3 mt-6 flex items-center text-sm font-black uppercase tracking-wider text-[#0EA5E9]"><Calculator className="mr-2" size={16} />Multiple {board.competition}</h4>
                      <div className="grid gap-3 lg:grid-cols-2">{board.multiples.map((bolletta) => <article key={bolletta.id} className="overflow-hidden rounded-2xl border border-[#0EA5E9]/25 bg-gradient-to-br from-[#1E293B] to-[#0F172A]"><button type="button" className="flex w-full items-center justify-between gap-3 p-4 text-left" onClick={() => toggleBolletta(bolletta.id)}><div><h5 className="font-black text-white">{bolletta.title}</h5><p className="mt-1 text-xs text-[#94A3B8]">{bolletta.description}</p></div><span className="shrink-0 font-black text-[#10B981]">{bolletta.totalOdds.toFixed(2)}</span></button><AnimatePresence>{expandedBolletta === bolletta.id && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-[#334155]/50 p-4"><div className="space-y-2">{bolletta.matches.map((match) => <div key={match.id} className="flex justify-between gap-3 rounded-lg bg-[#0F172A] p-3 text-xs"><div><p className="font-bold text-white">{match.match}</p><p className="mt-1 text-[#10B981]">{match.pick}</p></div><b className="text-white">{match.odds.toFixed(2)}</b></div>)}</div><a href={affiliateLink} target="_blank" rel="noopener noreferrer" className="mt-4 flex w-full items-center justify-center rounded-xl bg-[#0EA5E9] py-3 text-xs font-black text-white">VERIFICA QUOTE SU {bookmakerName}<ExternalLink size={15} className="ml-2" /></a></motion.div>}</AnimatePresence></article>)}</div>
                      {!board.multiples.length && <p className="mt-3 rounded-xl border border-dashed border-[#475569] p-3 text-xs text-[#94A3B8]">Servono almeno due match disponibili per creare una multipla {board.competition}.</p>}
                    </section>
                  )) : <div className="rounded-2xl border border-[#334155] bg-[#1E293B] p-8 text-center text-[#94A3B8]"><Target size={32} className="mx-auto mb-3 opacity-20" /><p>Nessun pronostico disponibile per questo campionato.</p></div>}
                </div>
              </>
            )}
          </motion.div>
