...
async function run() {
  // First clear any existing foreign players just in case
  await pool.query(`
    DELETE FROM players 
  ...
  for (const team of ALL_TEAMS) {
    ...
    if (data.players.length === 0) {
      console.log(`WARNING: 0 players found for ${team.name}`);
      continue;
    }
  ...
  console.log(`Done. Inserted ${totalPlayers} real players.`);
  process.exit(0);
}
