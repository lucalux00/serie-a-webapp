...
export async function GET(request: Request) {
  try {
    ...
    const { rows: roster } = await sql`SELECT player_name AS "playerName", role FROM fanta_rosters WHERE user_id = ${user.userId}`;
    ...
    const { rows } = await sql`
      SELECT p.name, COALESCE(p.position, p.role) AS role, COALESCE(t.name, p.team_id) AS "teamName"
    ...
    const suggestions = Object.keys(TARGET).map((role) => {
      ...
      const suggestions = rows.filter((p: any) => normalize(p.role) === role && !inRoster.has(p.name.toLowerCase())).slice(0, 7).map((p: any, index: number) => {
